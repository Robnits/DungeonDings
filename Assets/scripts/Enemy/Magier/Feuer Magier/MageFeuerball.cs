...
public class MageFeuerball : MonoBehaviour
{
    ...
    void Update()
    {
        if (player != null && Time.time - lastCastTime > castInterval)
        {
            ...
            StartCoroutine(DestroyAfterTime(feuerballInstance.gameObject)); // �bergeben Sie das GameObject des Projektils
        }
    }
   ...
   IEnumerator DestroyAfterTime()
    ...
    {
        yield return new WaitForSeconds(3f);
        Destroy(projectile); // Zerst�re das �bergebene Projektil-GameObject
    }
}